...
<body>
<h1>BookTitle or Author Results</H1>
<?php
...
if(empty($_SESSION['search'])){//checks if session variable search is empty
	header("Location: Search.php");//brings user to search page
}
$results_per_page = 5;
$page = 1;
$SERCH = $_SESSION["search"]; //sets search to the session variable search
	...
	$sql3 ="SELECT * FROM books WHERE BookTitle LIKE '%$SERCH%' OR Author 
	LIKE '%$SERCH%' LIMIT $start_from, $results_per_page";/*sql query that finds all the records related to 
	the search and puts a limit on them of where 
	the list will start and how many rows will be selected from that point*/
	$result = mysqli_query($conn, $sql3);//will run the sql staement in the database
	//diaplays the column headings
	echo "<table border ='1'>";
	echo "<tr>";
	echo "<th>ISBN</th>";
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		$page_query = "SELECT * FROM books WHERE BookTitle LIKE '%$SERCH%' OR Author LIKE '%$SERCH%' ";//sql query that selects all data from  
		//the database where the booktitle and author match the search
		...
		for($i=1; $i<=$total_page; $i++){
			echo "<a style ='font-size:30px; color: black; text-decoration: underline; ' href='DisplaySearch.php?page=".$i."'>".$i."</a>&nbsp; ";
		}
